...
SquareQDMType1PlaquetteClass = Literal["interior", "x_seam", "y_seam", "corner"]
SquareQDMType1AdaptiveSplitAxis = Literal["x", "y", "checkerboard"]
...
@dataclass(frozen=True, slots=True)
class SquareQDMType1PlaquetteResidualRecord:
    """Kinetic residual generated by one plaquette in a type-1 PEPS state."""

    plaquette_id: int
    anchor_cell: tuple[int, int]
    plaquette_class: SquareQDMType1PlaquetteClass
    component_norm_squared: float
    coherent_contribution: float
    max_residual: float
    n_nonzero_targets: int


@dataclass(frozen=True, slots=True)
class SquareQDMType1InterferenceClassRecord:
    """Coherent interference residual accumulated over one plaquette class."""

    plaquette_class: SquareQDMType1PlaquetteClass
    n_plaquettes: int
    residual_norm_squared: float
    incoherent_norm_squared: float
    coherent_contribution: float
    max_residual: float
    n_nonzero_targets: int
    ...
    def cancellation_fraction(self) -> float:
        if self.incoherent_norm_squared <= 0.0:
            return 1.0
        return float(
            1.0
            - min(
                1.0,
                max(0.0, self.residual_norm_squared / self.incoherent_norm_squared),
            )
        )


@dataclass(frozen=True, slots=True)
class SquareQDMType1ParameterSensitivity:
    """Gradient of one plaquette-class interference density by tensor entry."""
    ...
    def __post_init__(self) -> None:
        gradient = np.asarray(self.gradient, dtype=np.float64).reshape(-1)
        scores = np.asarray(self.scores, dtype=np.float64).reshape(-1)
        coordinates = np.asarray(self.entry_coordinates, dtype=np.int64)
        if gradient.shape != scores.shape:
            raise ValueError("gradient and scores must align.")
        if coordinates.shape != (gradient.size, 5):
            raise ValueError("entry_coordinates must have shape (n_entries, 5).")
    ...
    @property
    def ranked_entry_indices(self) -> npt.NDArray[np.int64]:
        return np.asarray(np.argsort(self.scores)[::-1], dtype=np.int64)

    def top_entry_indices(self, count: int = 12) -> npt.NDArray[np.int64]:
        count = max(0, min(int(count), self.scores.size))
        return self.ranked_entry_indices[:count]


@dataclass(frozen=True, slots=True)
class SquareQDMType1InterferenceDecomposition:
    """Plaquette- and seam-resolved decomposition of ``B psi``.

    ``component_norm_squared`` measures each plaquette before coherent summation.
    ``coherent_contribution = Re <r_p, r_total>`` partitions the exact total
    residual norm, including negative contributions from destructive
    interference.  The class records sum plaquettes before taking their norm,
    revealing whether interior, seam, or corner processes remain uncancelled.
    """

    total_norm_squared: float
    reconstruction_residual: float
    plaquette_records: tuple[SquareQDMType1PlaquetteResidualRecord, ...]
    class_records: tuple[SquareQDMType1InterferenceClassRecord, ...]
    metadata: Mapping[str, object] = field(default_factory=dict)

    def __post_init__(self) -> None:
        object.__setattr__(self, "total_norm_squared", float(self.total_norm_squared))
        object.__setattr__(self, "reconstruction_residual", float(self.reconstruction_residual))
        object.__setattr__(self, "plaquette_records", tuple(self.plaquette_records))
        object.__setattr__(self, "class_records", tuple(self.class_records))
        object.__setattr__(self, "metadata", dict(self.metadata))

    @property
    def by_class(self) -> dict[SquareQDMType1PlaquetteClass, SquareQDMType1InterferenceClassRecord]:
        return {record.plaquette_class: record for record in self.class_records}

    @property
    def dominant_seam_class(self) -> SquareQDMType1PlaquetteClass:
        seam_records = [
            record
            for record in self.class_records
            if record.plaquette_class in {"x_seam", "y_seam", "corner"}
        ]
        if not seam_records:
            return "interior"
        return max(seam_records, key=lambda record: record.residual_norm_squared).plaquette_class

    @property
    def incoherent_norm_squared(self) -> float:
        return float(sum(record.component_norm_squared for record in self.plaquette_records))

    @property
    def global_cancellation_fraction(self) -> float:
        if self.incoherent_norm_squared <= 0.0:
            return 1.0
        return float(
            1.0 - min(1.0, max(0.0, self.total_norm_squared / self.incoherent_norm_squared))
        )


def _square_qdm_type1_plaquette_class(
    anchor_cell: Sequence[int],
    tile_shape: tuple[int, int],
) -> SquareQDMType1PlaquetteClass:
    x, y = (int(anchor_cell[0]), int(anchor_cell[1]))
    tile_lx, tile_ly = tile_shape
    crosses_x = x % tile_lx == tile_lx - 1
    crosses_y = y % tile_ly == tile_ly - 1
...
def _square_qdm_type1_local_kinetic_blocks(
    problem: "SquareQDMType1PEPSFiniteClusterProblem",
) -> tuple[tuple[int, tuple[int, int], SquareQDMType1PlaquetteClass, scipy_sparse.csr_array], ...]:
    """Build rectangular ``K_p`` blocks in the exact basis used by ``problem``."""
    builder = str(problem.base_problem.metadata.get("builder", "sparse"))
    basis_solver = str(problem.base_problem.metadata.get("basis_solver", "dfs"))
    model = problem.base_problem.model
    build_result = model.build(basis_solver=basis_solver, builder=builder)
    rebuilt_states = np.asarray(build_result.basis.states, dtype=np.int8)
    if not np.array_equal(rebuilt_states, problem.base_problem.basis_states):
        raise RuntimeError("Rebuilt local-term basis order does not match the PEPS problem.")
    blocks = []
    for descriptor in model.local_term_descriptors(operator_kind="kinetic"):
        ...
        block = scipy_sparse.csr_array(
            matrix[
                problem.opposite_basis_indices[:, None],
                problem.target_basis_indices,
            ],
            dtype=np.complex128,
        )
        anchor = tuple(int(value) for value in model.lattice.plaquette_anchor_cell(plaquette_id))
        if len(anchor) < 2:
            raise ValueError("Square-QDM plaquette anchors must contain x and y coordinates.")
        anchor_xy = (anchor[0], anchor[1])
        blocks.append(
            (
                plaquette_id,
                anchor_xy,
                _square_qdm_type1_plaquette_class(anchor_xy, problem.tile_basis.tile_shape),
                block,
            )
        )
    return tuple(blocks)
...
def _square_qdm_type1_group_matrix(
    problem: "SquareQDMType1PEPSFiniteClusterProblem",
    plaquette_class: SquareQDMType1PlaquetteClass,
) -> scipy_sparse.csr_array:
    shape = problem.kinetic_interference_matrix.shape
    result = scipy_sparse.csr_array(shape, dtype=np.complex128)
    for _, _, current_class, block in _square_qdm_type1_local_kinetic_blocks(problem):
        if current_class == plaquette_class:
            result = result + block
    return scipy_sparse.csr_array(result, dtype=np.complex128)


def _state_product_jacobian(
    parameters: npt.NDArray[np.complex128],
    parameter_indices: npt.NDArray[np.int64],
) -> npt.NDArray[np.complex128]:
    """Jacobian of products of selected tensor entries, including exact zeros."""
    indices = np.asarray(parameter_indices, dtype=np.int64)
    if indices.ndim != 2:
        raise ValueError("parameter_indices must be two-dimensional.")
    n_states, n_tiles = indices.shape
    jacobian = np.zeros((n_states, parameters.size), dtype=np.complex128)
    rows = np.arange(n_states, dtype=np.int64)
    for tile_index in range(n_tiles):
        if n_tiles == 1:
            excluded_product = np.ones(n_states, dtype=np.complex128)
        else:
            other_indices = np.delete(indices, tile_index, axis=1)
            excluded_product = np.prod(parameters[other_indices], axis=1, dtype=np.complex128)
        np.add.at(
            jacobian,
            (rows, indices[:, tile_index]),
            excluded_product,
        )
    return jacobian
...
class SquareQDMType1PEPSFiniteClusterProblem:
    ...
    def interference_decomposition(
        self,
        parameters: npt.ArrayLike,
    ) -> SquareQDMType1InterferenceDecomposition:
        """Resolve the type-1 kinetic residual by plaquette and tile seam."""
        state = self.native_state_vector(parameters, normalize=True)
        total_residual = np.asarray(
            self.kinetic_interference_matrix @ state,
            dtype=np.complex128,
        ).reshape(-1)
        local_blocks = _square_qdm_type1_local_kinetic_blocks(self)
        plaquette_records: list[SquareQDMType1PlaquetteResidualRecord] = []
        class_vectors: dict[SquareQDMType1PlaquetteClass, npt.NDArray[np.complex128]] = {
            label: np.zeros(total_residual.shape, dtype=np.complex128)
            for label in ("interior", "x_seam", "y_seam", "corner")
        }
        class_incoherent = {label: 0.0 for label in class_vectors}
        class_counts = {label: 0 for label in class_vectors}
        reconstructed = np.zeros(total_residual.shape, dtype=np.complex128)
        for plaquette_id, anchor, plaquette_class, block in local_blocks:
            residual = np.asarray(block @ state, dtype=np.complex128).reshape(-1)
            reconstructed += residual
            class_vectors[plaquette_class] += residual
            component_norm = float(np.vdot(residual, residual).real)
            class_incoherent[plaquette_class] += component_norm
            class_counts[plaquette_class] += 1
            plaquette_records.append(
                SquareQDMType1PlaquetteResidualRecord(
                    plaquette_id=plaquette_id,
                    anchor_cell=anchor,
                    plaquette_class=plaquette_class,
                    component_norm_squared=component_norm,
                    coherent_contribution=float(np.vdot(residual, total_residual).real),
                    max_residual=float(np.max(np.abs(residual), initial=0.0)),
                    n_nonzero_targets=int(np.count_nonzero(np.abs(residual) > self.tolerance)),
                )
            )
        class_records = []
        for plaquette_class in ("interior", "x_seam", "y_seam", "corner"):
            residual = class_vectors[plaquette_class]
            class_records.append(
                SquareQDMType1InterferenceClassRecord(
                    plaquette_class=plaquette_class,
                    n_plaquettes=class_counts[plaquette_class],
                    residual_norm_squared=float(np.vdot(residual, residual).real),
                    incoherent_norm_squared=float(class_incoherent[plaquette_class]),
                    coherent_contribution=float(np.vdot(residual, total_residual).real),
                    max_residual=float(np.max(np.abs(residual), initial=0.0)),
                    n_nonzero_targets=int(np.count_nonzero(np.abs(residual) > self.tolerance)),
                )
            )
        return SquareQDMType1InterferenceDecomposition(
            total_norm_squared=float(np.vdot(total_residual, total_residual).real),
            reconstruction_residual=float(np.linalg.norm(total_residual - reconstructed)),
            plaquette_records=tuple(plaquette_records),
            class_records=tuple(class_records),
            metadata={
                "model_shape": self.base_problem.metadata.get("model_shape"),
                "tile_shape": self.tile_basis.tile_shape,
                "n_target_states": int(state.size),
            },
        )

    def interference_parameter_sensitivity(
        self,
        parameters: npt.ArrayLike,
        plaquette_class: SquareQDMType1PlaquetteClass,
        ...
        The derivative is evaluated analytically from the product-state
        Jacobian, so entries with zero amplitude are handled exactly.  Large
        scores identify local boundary sectors whose amplitudes most strongly
        control the selected uncancelled interference channel.
        """
        values = np.asarray(parameters, dtype=np.complex128).reshape(-1)
        if values.size != self.tile_basis.n_entries:
            raise ValueError(f"parameters must have size {self.tile_basis.n_entries}.")
        indices = self.target_entry_parameter_indices
        state = np.prod(values[indices], axis=1, dtype=np.complex128)
        norm_squared = float(np.vdot(state, state).real)
        if norm_squared <= 0.0:
            raise ValueError("The PEPS is zero in the selected chiral sector.")
        jacobian = _state_product_jacobian(values, indices)
        group_matrix = _square_qdm_type1_group_matrix(self, plaquette_class)
        residual = np.asarray(group_matrix @ state, dtype=np.complex128).reshape(-1)
        residual_jacobian = np.asarray(group_matrix @ jacobian, dtype=np.complex128)
        numerator = float(np.vdot(residual, residual).real)
        overlap_residual = np.real(np.conjugate(residual) @ residual_jacobian)
        overlap_norm = np.real(np.conjugate(state) @ jacobian)
        gradient = (
            2.0 * overlap_residual / norm_squared
            - 2.0 * numerator * overlap_norm / (norm_squared**2)
        ) / self.n_plaquettes
        gradient = np.asarray(gradient, dtype=np.float64)
        return SquareQDMType1ParameterSensitivity(
            plaquette_class=plaquette_class,
            gradient=gradient,
            scores=np.abs(gradient),
            entry_coordinates=self.tile_basis.entry_coordinates,
            loss=numerator / norm_squared / self.n_plaquettes,
            metadata={
                "model_shape": self.base_problem.metadata.get("model_shape"),
                "tile_shape": self.tile_basis.tile_shape,
            },
...
@dataclass(frozen=True, slots=True)
class SquareQDMType1AdaptiveParameterization:
    """Targeted multi-tensor enlargement of the compact type-1 PEPS ansatz.

    Most of the 108 locally allowed entries remain shared across the whole
    lattice.  Only ``selected_entry_indices`` receive independent copies on a
    period-two tile sublattice.  This tests whether a specific seam obstruction
    requires a larger unit cell without duplicating the complete tensor.
    """

    tile_basis: SquareQDMRectangularTileTensorBasis
    selected_entry_indices: npt.NDArray[np.int64]
    split_axis: SquareQDMType1AdaptiveSplitAxis
    period: int = 2
    metadata: Mapping[str, object] = field(default_factory=dict)

    def __post_init__(self) -> None:
        selected = np.asarray(self.selected_entry_indices, dtype=np.int64).reshape(-1)
        selected = np.unique(selected)
        if selected.size == 0:
            raise ValueError("At least one tensor entry must be selected for enlargement.")
        if np.any(selected < 0) or np.any(selected >= self.tile_basis.n_entries):
            raise IndexError("selected_entry_indices contains an invalid tensor entry.")
        if self.split_axis not in {"x", "y", "checkerboard"}:
            raise ValueError("split_axis must be 'x', 'y', or 'checkerboard'.")
        if int(self.period) != 2:
            raise ValueError("The first adaptive implementation supports period=2 only.")
        object.__setattr__(self, "selected_entry_indices", selected)
        object.__setattr__(self, "period", 2)
        object.__setattr__(self, "metadata", dict(self.metadata))

    @property
    def n_classes(self) -> int:
        return 4 if self.split_axis == "checkerboard" else 2

    @property
    def n_parameters(self) -> int:
        return int(
            self.tile_basis.n_entries + self.selected_entry_indices.size * (self.n_classes - 1)
        )

    @property
    def parameter_table(self) -> npt.NDArray[np.int64]:
        table = np.repeat(
            np.arange(self.tile_basis.n_entries, dtype=np.int64)[:, None],
            self.n_classes,
            axis=1,
        )
        next_index = self.tile_basis.n_entries
        for entry_index in self.selected_entry_indices:
            for class_index in range(1, self.n_classes):
                table[int(entry_index), class_index] = next_index
                next_index += 1
        return table

    def tile_class(self, tile_coordinate: tuple[int, int]) -> int:
        tile_x, tile_y = (int(tile_coordinate[0]), int(tile_coordinate[1]))
        if self.split_axis == "x":
            return tile_x % 2
        if self.split_axis == "y":
            return tile_y % 2
        return tile_x % 2 + 2 * (tile_y % 2)

    def validate_problem(self, problem: SquareQDMType1PEPSFiniteClusterProblem) -> None:
        if problem.tile_basis.n_entries != self.tile_basis.n_entries or not np.array_equal(
            ...
            raise ValueError("Adaptive parameterization and problem use different tile bases.")
        if self.split_axis in {"x", "checkerboard"} and problem.base_problem.n_tiles_x % 2:
            raise ValueError("The cluster needs an even number of x tiles for this enlargement.")
        if self.split_axis in {"y", "checkerboard"} and problem.base_problem.n_tiles_y % 2:
            raise ValueError("The cluster needs an even number of y tiles for this enlargement.")

    def lift_parameters(self, parameters: npt.ArrayLike) -> npt.NDArray[np.float64]:
        values = np.asarray(parameters, dtype=np.complex128).reshape(-1)
        if values.size != self.tile_basis.n_entries:
            raise ValueError(f"parameters must have size {self.tile_basis.n_entries}.")
        if np.max(np.abs(values.imag), initial=0.0) > 1.0e-12:
            raise ValueError("The adaptive Autograd prototype accepts real parameters only.")
        table = self.parameter_table
        expanded = np.zeros(self.n_parameters, dtype=np.float64)
        expanded[: self.tile_basis.n_entries] = values.real
        for entry_index in self.selected_entry_indices:
            for class_index in range(1, self.n_classes):
                expanded[table[int(entry_index), class_index]] = values.real[int(entry_index)]
        return expanded

    def target_parameter_indices(
        self,
        problem: SquareQDMType1PEPSFiniteClusterProblem,
    ) -> npt.NDArray[np.int64]:
        self.validate_problem(problem)
        table = self.parameter_table
        result = np.empty_like(problem.target_entry_parameter_indices)
        for tile_index, coordinate in enumerate(problem.base_problem.tile_coordinates):
            class_index = self.tile_class(coordinate)
            result[:, tile_index] = table[
                problem.target_entry_parameter_indices[:, tile_index],
                class_index,
            ]
        return result

    def class_parameter_indices(self, class_index: int) -> npt.NDArray[np.int64]:
        class_index = int(class_index)
        if class_index < 0 or class_index >= self.n_classes:
            raise IndexError("class_index is outside the adaptive unit cell.")
        return np.asarray(self.parameter_table[:, class_index], dtype=np.int64)


@dataclass(frozen=True, slots=True)
class SquareQDMType1AdaptivePEPSFiniteClusterProblem:
    """One finite cluster evaluated with a targeted enlarged unit cell."""
    ...
    def __post_init__(self) -> None:
        self.parameterization.validate_problem(self.base_problem)
        indices = np.asarray(self.parameter_indices, dtype=np.int64)
        expected = self.base_problem.target_entry_parameter_indices.shape
        if indices.shape != expected:
            raise ValueError("parameter_indices has the wrong shape.")
        if np.any(indices < 0) or np.any(indices >= self.parameterization.n_parameters):
            raise IndexError("parameter_indices contains an invalid adaptive parameter.")
        object.__setattr__(self, "parameter_indices", indices.copy())
    ...
    def from_problem(
        ...
        return cls(
            ...
            parameter_indices=parameterization.target_parameter_indices(problem),
        )

    @property
    def n_plaquettes(self) -> int:
        return self.base_problem.n_plaquettes
    ...
    def state_vector(
        ...
        *,
        ...
        values = np.asarray(parameters, dtype=np.complex128).reshape(-1)
        if values.size != self.parameterization.n_parameters:
            raise ValueError(f"parameters must have size {self.parameterization.n_parameters}.")
        state = np.prod(values[self.parameter_indices], axis=1, dtype=np.complex128)
        if normalize:
            norm = float(np.linalg.norm(state))
            if norm == 0.0:
                raise ValueError("The adaptive PEPS is zero in the selected chiral sector.")
            state = state / norm
        return np.asarray(state, dtype=np.complex128)

    def diagnose(self, parameters: npt.ArrayLike) -> SquareQDMType1PEPSResidualReport:
        state = self.state_vector(parameters, normalize=False)
        state_norm = float(np.linalg.norm(state))
        if state_norm == 0.0:
            return SquareQDMType1PEPSResidualReport(
                norm_before_projection=0.0,
                ...
                retained_chiral_weight=1.0,
                discarded_chiral_weight=0.0,
                target_chiral_label=self.base_problem.target_chiral_label,
                kinetic_interference_norm=float("inf"),
                kinetic_interference_density=float("inf"),
                potential_mean=0.0,
                potential_variance=float("inf"),
                ...
                n_nonzero_interference_targets=0,
                nonzero_projected_amplitudes=0,
                hilbert_dimension=self.base_problem.base_problem.hilbert_dimension,
                target_potential_value=self.base_problem.target_potential_value,
                target_potential_residual=None,
            )
        state = state / state_norm
        interference = np.asarray(
            self.base_problem.kinetic_interference_matrix @ state,
            dtype=np.complex128,
        ).reshape(-1)
        kinetic_norm = float(np.vdot(interference, interference).real)
        probabilities = np.abs(state) ** 2
        potential = self.base_problem.target_potential_values
        potential_mean = float(np.sum(probabilities * potential))
        potential_variance = float(np.sum(probabilities * (potential - potential_mean) ** 2))
        target_residual = None
        if self.base_problem.target_potential_value is not None:
            target_residual = float(
                np.sum(
                    probabilities
                    * (potential - float(self.base_problem.target_potential_value)) ** 2
                )
            )
        return SquareQDMType1PEPSResidualReport(
            norm_before_projection=state_norm,
            ...
            discarded_chiral_weight=0.0,
            target_chiral_label=self.base_problem.target_chiral_label,
            kinetic_interference_norm=kinetic_norm,
            kinetic_interference_density=kinetic_norm / self.n_plaquettes,
            potential_mean=potential_mean,
            potential_variance=potential_variance,
            potential_variance_density=potential_variance / self.n_plaquettes,
            total_variance=kinetic_norm + potential_variance,
            objective=(kinetic_norm + self.base_problem.potential_weight * potential_variance)
            / self.n_plaquettes,
            max_interference_residual=float(np.max(np.abs(interference), initial=0.0)),
            n_nonzero_interference_targets=int(
                np.count_nonzero(np.abs(interference) > self.base_problem.tolerance)
            ),
            nonzero_projected_amplitudes=int(
                np.count_nonzero(np.abs(state) > self.base_problem.tolerance)
            ),
            hilbert_dimension=self.base_problem.base_problem.hilbert_dimension,
            target_potential_value=self.base_problem.target_potential_value,
            target_potential_residual=target_residual,
...
@dataclass(frozen=True, slots=True)
class SquareQDMType1AdaptiveJointOptimizationResult:
    """Joint optimization result for a targeted enlarged type-1 unit cell."""
...
@dataclass(frozen=True, slots=True)
class SquareQDMType1AdaptiveJointClusterProblem:
    """Shared targeted multi-tensor objective over several exact clusters."""

    problems: Mapping[str, SquareQDMType1AdaptivePEPSFiniteClusterProblem]
    ...
    def __post_init__(self) -> None:
        ...
        if not problems:
            raise ValueError("At least one adaptive type-1 cluster problem is required.")
        first = next(iter(problems.values()))
        for label, problem in problems.items():
            if problem.parameterization is not first.parameterization:
                raise ValueError(f"Cluster {label!r} uses a different adaptive parameterization.")
    ...
    @property
    def parameterization(self) -> SquareQDMType1AdaptiveParameterization:
        return next(iter(self.problems.values())).parameterization
    ...
    def diagnose(self, parameters: npt.ArrayLike) -> SquareQDMType1ClusterValidationReport:
        records = tuple(
            SquareQDMType1ClusterValidationRecord(
                ...
                report=problem.diagnose(parameters),
            )
    ...
    def loss(self, parameters: npt.ArrayLike) -> float:
        return self.diagnose(parameters).objective
    ...
    def loss_and_gradient_autograd(
        ...
        cluster_data = tuple(
            (
                anp.asarray(problem.parameter_indices, dtype=np.int64),
                anp.asarray(problem.base_problem.kinetic_interference_matrix.toarray().real),
                anp.asarray(problem.base_problem.target_potential_values),
        ...
        potential_weight = float(self.potential_weight)
        aggregate_epsilon = 1.0e-120

        def objective(compact_parameters: Any) -> Any:
            kinetic_terms = []
            potential_terms = []
            weights = []
            for indices, interference, potential, n_plaquettes, weight in cluster_data:
                state = anp.prod(compact_parameters[indices], axis=1)
                norm_squared = anp.sum(state * state) + 1.0e-30
    ...
    def loss_and_gradient_exact(
        self,
        parameters: npt.ArrayLike,
    ) -> tuple[float, npt.NDArray[np.float64]]:
        """Evaluate the adaptive joint loss and analytic real gradient.

        This path is independent of Autograd and quimb.  It is useful for
        regression tests and for diagnosing the targeted enlargement before a
        more expensive tensor-network optimization is launched.
        """
        values = np.asarray(parameters, dtype=np.complex128).reshape(-1)
        if values.size != self.parameterization.n_parameters:
            raise ValueError(f"parameters must have size {self.parameterization.n_parameters}.")
        if np.max(np.abs(values.imag), initial=0.0) > self.tolerance:
            raise ValueError("The analytic adaptive gradient accepts real parameters only.")
        values_real = np.asarray(values.real, dtype=np.float64)
        power = float(self.aggregation_power)
        total_weight = float(sum(self.cluster_weights.values()))
        kinetic_values = []
        potential_values = []
        ...
        weights = []
        for label, problem in self.problems.items():
            indices = problem.parameter_indices
            state = np.prod(values_real[indices], axis=1, dtype=np.float64)
            jacobian = _state_product_jacobian(
                values_real.astype(np.complex128),
                indices,
            ).real
            norm_squared = float(np.dot(state, state)) + 1.0e-30
            interference = np.asarray(
                problem.base_problem.kinetic_interference_matrix.toarray().real,
                dtype=np.float64,
            )
            residual = interference @ state
            residual_jacobian = interference @ jacobian
            kinetic_numerator = float(np.dot(residual, residual))
            kinetic_density = kinetic_numerator / norm_squared / problem.n_plaquettes
            kinetic_gradient = (
                2.0 * residual @ residual_jacobian / norm_squared
                - 2.0 * kinetic_numerator * (state @ jacobian) / (norm_squared**2)
            ) / problem.n_plaquettes

            potential = problem.base_problem.target_potential_values
            probabilities = state * state / norm_squared
            potential_mean = float(np.dot(probabilities, potential))
            centered_squared = (potential - potential_mean) ** 2
            potential_variance = float(np.dot(probabilities, centered_squared))
            potential_density = potential_variance / problem.n_plaquettes
            potential_gradient = (
                2.0
                * (((centered_squared - potential_variance) * state) @ jacobian)
                / norm_squared
                / problem.n_plaquettes
            )
            kinetic_values.append(kinetic_density)
            potential_values.append(potential_density)
            kinetic_gradients.append(np.asarray(kinetic_gradient, dtype=np.float64))
            potential_gradients.append(np.asarray(potential_gradient, dtype=np.float64))
            weights.append(float(self.cluster_weights[label]))

        weight_array = np.asarray(weights, dtype=np.float64)
        kinetic_array = np.asarray(kinetic_values, dtype=np.float64)
        potential_array = np.asarray(potential_values, dtype=np.float64)
        kinetic_gradient_array = np.stack(kinetic_gradients, axis=0)
        potential_gradient_array = np.stack(potential_gradients, axis=0)
        aggregate_epsilon = 1.0e-120

        def aggregate(
            values_array: npt.NDArray[np.float64],
            gradients_array: npt.NDArray[np.float64],
        ) -> tuple[float, npt.NDArray[np.float64]]:
            powered_mean = (
                float(np.dot(weight_array, values_array**power)) / total_weight + aggregate_epsilon
            )
            aggregate_value = powered_mean ** (1.0 / power)
            factors = weight_array * values_array ** (power - 1.0) / total_weight
            aggregate_gradient = powered_mean ** (1.0 / power - 1.0) * np.sum(
                factors[:, None] * gradients_array, axis=0
            )
            shifted_value = aggregate_value - aggregate_epsilon ** (1.0 / power)
            return float(shifted_value), np.asarray(aggregate_gradient, dtype=np.float64)

        kinetic_aggregate, kinetic_gradient = aggregate(
            kinetic_array,
            kinetic_gradient_array,
        )
        potential_aggregate, potential_gradient = aggregate(
            potential_array,
            potential_gradient_array,
        )
        return (
            kinetic_aggregate + self.potential_weight * potential_aggregate,
            kinetic_gradient + self.potential_weight * potential_gradient,
        )
    ...
    def optimize_with_scipy(
        ...
        *,
        ...
        if values.size != self.parameterization.n_parameters:
            raise ValueError(f"parameters must have size {self.parameterization.n_parameters}.")
        rng = np.random.default_rng(seed)
        initial = values + float(noise_scale) * rng.normal(size=values.size)
        norm = float(np.linalg.norm(initial))
        if norm == 0.0:
            raise ValueError("Cannot optimize a zero adaptive parameter vector.")
        initial = initial / norm
        initial_validation = self.diagnose(initial)
        losses: list[float] = []

        def objective(
            compact_parameters: npt.NDArray[np.float64],
        ) -> tuple[float, npt.NDArray[np.float64]]:
            loss, gradient = self.loss_and_gradient_exact(compact_parameters)
        ...
        result = scipy_optimize.minimize(
            ...
            method=optimizer,
            jac=True,
            options={"maxiter": int(max_steps)},
        )
        optimized = np.asarray(result.x, dtype=np.float64).reshape(-1)
        return SquareQDMType1AdaptiveJointOptimizationResult(
            initial_parameters=initial,
            optimized_parameters=optimized,
            loss_history=tuple(losses),
            initial_validation=initial_validation,
            final_validation=self.diagnose(optimized),
            parameterization=self.parameterization,
            requested_steps=int(max_steps),
            optimizer=str(optimizer),
            autodiff_backend="analytic",
            metadata={
                "noise_scale": float(noise_scale),
                "seed": seed,
    ...
    def make_quimb_optimizer(
        ...
        **backend_options: object,
        ...
        if values.size != self.parameterization.n_parameters:
            raise ValueError(f"parameters must have size {self.parameterization.n_parameters}.")
        if np.max(np.abs(values.imag), initial=0.0) > self.tolerance:
            raise ValueError("The adaptive quimb optimizer accepts real parameters only.")
        tensor = qtn.PTensor(
            lambda compact_parameters: compact_parameters,
            np.asarray(values.real, dtype=np.float64),
            inds=("adaptive_parameter",),
            tags={"ADAPTIVE_PARAMETERS"},
        )
        network = qtn.TensorNetwork([tensor])
        cluster_data = tuple(
            {
                "parameter_indices": problem.parameter_indices,
                "kinetic": np.asarray(
                    problem.base_problem.kinetic_interference_matrix.toarray().real,
                    dtype=np.float64,
                ),
                "potential": np.asarray(
                    problem.base_problem.target_potential_values,
                    dtype=np.float64,
                ),
        ...
        return qtn.TNOptimizer(
            network,
            _quimb_type1_adaptive_joint_loss,
            loss_constants={
    ...
    def optimize_with_quimb(
        ...
        **backend_options: object,
        ...
        optimized = np.asarray(
            optimized_network["ADAPTIVE_PARAMETERS"].params,
            dtype=np.float64,
        ).reshape(-1)
        return SquareQDMType1AdaptiveJointOptimizationResult(
            initial_parameters=initial,
...
def _quimb_type1_adaptive_joint_loss(
    tensor_network: object,
    *,
    ...
    """Autodiff-compatible joint loss for a targeted enlarged unit cell."""
    import autoray as ar

    parameters = tensor_network["ADAPTIVE_PARAMETERS"].data
    power = float(aggregation_power)
    total_weight = float(sum(float(data["weight"]) for data in cluster_data))
    aggregate_epsilon = 1.0e-120
    kinetic_sum = 0.0
    potential_sum = 0.0
    for data in cluster_data:
        state = ar.do("prod", parameters[data["parameter_indices"]], axis=1)
        norm_squared = ar.do("sum", state * state) + 1.0e-30
...
def build_square_qdm_type1_adaptive_parameterization(
    problem: SquareQDMType1PEPSFiniteClusterProblem,
    parameters: npt.ArrayLike,
    *,
    plaquette_class: SquareQDMType1PlaquetteClass | None = None,
    split_axis: SquareQDMType1AdaptiveSplitAxis | Literal["auto"] = "auto",
    max_selected_entries: int = 12,
    ...
    values = np.asarray(parameters, dtype=np.complex128).reshape(-1)
    if values.size != problem.tile_basis.n_entries:
        raise ValueError(f"parameters must have size {problem.tile_basis.n_entries}.")
    decomposition = problem.interference_decomposition(values)
    selected_class = (
        decomposition.dominant_seam_class if plaquette_class is None else plaquette_class
    )
    if split_axis == "auto":
        inferred_axis: SquareQDMType1AdaptiveSplitAxis
        if selected_class == "x_seam":
            inferred_axis = "x"
        elif selected_class == "y_seam":
            inferred_axis = "y"
        else:
            inferred_axis = "checkerboard"
    else:
        inferred_axis = split_axis
    sensitivity_values = values
    sensitivity = problem.interference_parameter_sensitivity(
        sensitivity_values,
        selected_class,
    )
    if not np.any(sensitivity.scores > problem.tolerance) and probe_scale > 0.0:
        sensitivity_values = problem.base_problem.perturb_parameters(
            values,
            scale=probe_scale,
            seed=seed,
            normalize=True,
    ...
    positive = sensitivity.ranked_entry_indices[
        sensitivity.scores[sensitivity.ranked_entry_indices] > problem.tolerance
    ]
    count = max(1, min(int(max_selected_entries), positive.size))
    if positive.size == 0:
        raise ValueError("No parameter sensitivity was found for the selected seam class.")
    selected = positive[:count]
    return SquareQDMType1AdaptiveParameterization(
        tile_basis=problem.tile_basis,
        selected_entry_indices=selected,
        split_axis=inferred_axis,
        metadata={
            "plaquette_class": selected_class,
            "sensitivity_loss": sensitivity.loss,
            "sensitivity_scores": sensitivity.scores[selected].tolist(),
...
def build_square_qdm_type1_adaptive_joint_cluster_problem(
    ...
    *,
    ...
    adaptive = {
        str(label): SquareQDMType1AdaptivePEPSFiniteClusterProblem.from_problem(
            problem,
            parameterization,
        )
        for label, problem in problems.items()
    }
    return SquareQDMType1AdaptiveJointClusterProblem(
        problems=adaptive,
