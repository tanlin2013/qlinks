...
class McwfOptions:
    ...
    event_no_jump_propagator: str = "first_order"
    ...
    def validate(self) -> None:
        ...
        if self.event_no_jump_propagator not in {"first_order", "krylov"}:
            raise ValueError("event_no_jump_propagator must be either 'first_order' or 'krylov'.")
    ...
    for time_index in range(times.size - 1):
        ...
        if options.use_event_driven_jumps:
            ...
            states, event_survival_thresholds = _advance_state_matrix_event_driven_numpy(
                ...
                event_no_jump_propagator=options.event_no_jump_propagator,
...
def _no_jump_endpoint_norm_squares(states: ArrayC) -> NDArray[np.float64]:
    """Return column-wise squared norms for an unnormalized state matrix."""
    norm_squares = np.einsum("ij,ij->j", states.conj(), states, optimize=True).real
    return np.asarray(norm_squares, dtype=np.float64)


def _krylov_no_jump_propagate_state_matrix_numpy(
    *,
    states: ArrayC,
    effective_hamiltonian_matrix: Any,
    step_sizes: NDArray[np.float64],
    ...
    """Apply ``exp(-1j H_eff dt)`` to state columns with possibly different ``dt``.

    SciPy's ``expm_multiply`` accepts a dense matrix of right-hand sides only
    for a common time step.  Event-driven MCWF columns can have different
    segment lengths, so this helper groups columns by equal step size.  The
    grouping counter is intentionally exposed: if the number of Krylov groups
    is comparable to the number of columns, this experimental propagator will
    likely be slower than the first-order path.
    """
    step_sizes = np.asarray(step_sizes, dtype=np.float64)
    if step_sizes.shape != (states.shape[1],):
        raise ValueError("step_sizes must have one entry per state column.")

    propagated_states = np.empty_like(states)
    positive_step_mask = step_sizes > 0.0
    if np.any(~positive_step_mask):
        propagated_states[:, ~positive_step_mask] = states[:, ~positive_step_mask]

    if not np.any(positive_step_mask):
        return propagated_states

    # Round only for grouping numerical near-duplicates generated by vectorized
    # segment-limit arithmetic.  The actual step value used for propagation is
    # the first original value in each group, not the rounded key.
    positive_indices = np.flatnonzero(positive_step_mask)
    group_keys = np.round(step_sizes[positive_indices], decimals=14)
    unique_group_keys = np.unique(group_keys)
    _add_counter(timing_collector, "mcwf.count.krylov_step_groups", float(unique_group_keys.size))
    _add_counter(timing_collector, "mcwf.count.krylov_columns", float(positive_indices.size))

    start = _perf_counter()
    for group_key in unique_group_keys:
        group_positions = positive_indices[group_keys == group_key]
        group_step = float(step_sizes[group_positions[0]])
        if group_positions.size == 1:
            right_hand_side = states[:, group_positions]
        else:
            right_hand_side = states[:, group_positions]
        propagated_states[:, group_positions] = scipy_sparse_linalg.expm_multiply(
            (-1j * group_step) * effective_hamiltonian_matrix,
            right_hand_side,
        )

    elapsed = _perf_counter() - start
    _add_timing(timing_collector, "mcwf.krylov_no_jump_propagation", elapsed)
    return propagated_states
...
def _advance_state_matrix_event_driven_numpy(
    *,
    ...
    if event_no_jump_propagator not in {"first_order", "krylov"}:
        raise ValueError("event_no_jump_propagator must be either 'first_order' or 'krylov'.")
    ...
    if not jump_operators:
        start = _perf_counter()
        if event_no_jump_propagator == "krylov":
            next_states = _krylov_no_jump_propagate_state_matrix_numpy(
                states=states,
                effective_hamiltonian_matrix=effective_hamiltonian_matrix,
                step_sizes=np.full(states.shape[1], step_size, dtype=np.float64),
                timing_collector=timing_collector,
            )
        else:
            next_states = states - 1j * step_size * (effective_hamiltonian_matrix @ states)
    ...
    while active_indices.size:
        ...
        if event_no_jump_propagator == "krylov":
            next_active_states = _krylov_no_jump_propagate_state_matrix_numpy(
                states=active_states,
                effective_hamiltonian_matrix=effective_hamiltonian_matrix,
                step_sizes=segment_steps,
                timing_collector=timing_collector,
            )
        else:
        ...
        start = _perf_counter()
        if event_no_jump_propagator == "krylov":
            norm_squares = _no_jump_endpoint_norm_squares(next_active_states)
            krylov_endpoint_crossing_mask = norm_squares <= active_thresholds
            if np.any(krylov_endpoint_crossing_mask & ~crossing_mask):
                crossing_mask = crossing_mask | krylov_endpoint_crossing_mask
        else:
