...
FIXED_L12_BUDGETS="${QLINKS_QDM_FIXED_O1_L12_BUDGETS:-auto}"
FIXED_L12_TOLERANCE="${QLINKS_QDM_FIXED_O1_L12_TOLERANCE:-1e-8}"
FIXED_L12_MAX_BUDGET="${QLINKS_QDM_FIXED_O1_MAX_BUDGET:-8192}"
FIXED_L12_RESIDUAL_TOLERANCE="${QLINKS_QDM_FIXED_O1_RESIDUAL_TOLERANCE:-1e-6}"
FIXED_L12_OBSERVABLE_TOLERANCE="${QLINKS_QDM_FIXED_O1_OBSERVABLE_TOLERANCE:-5e-5}"
...
case "${STAGE}" in
    ...
    target-block-refine)
        ...
        SOLVE_POLICY="target E=12 only; closed audit lane; no broad-window acceptance test"
        ;;
    fixed-O1-pilot)
        JOB_COMMAND=(
        ...
        ;;
    fixed-O1-L12-spectrum)
        JOB_COMMAND=(
            python experimental/jobs/qdm_sec7_fixed_o1_l12_spectrum.py
            --output-dir "${OUTPUT_DATA_DIR}"
            --cache-root "${CACHE_ROOT}"
        ...
        ;;
    fixed-O1-L12-observables)
        JOB_COMMAND=(
            python experimental/jobs/qdm_sec7_fixed_o1_l12_observables.py
            --primme-data-dir "${PRIMME_DATA_DIR}"
            --output-dir "${OUTPUT_DATA_DIR}"
            --cache-root "${CACHE_ROOT}"
            --residual-tolerance "${FIXED_L12_RESIDUAL_TOLERANCE}"
            --observable-budget-tolerance "${FIXED_L12_OBSERVABLE_TOLERANCE}"
        )
        SOLVE_POLICY="solver-free checkpoint postprocessing; raw A/Z and stripe covariance with cleaned companions"
        ;;
    fixed-O1-three-size)
        JOB_COMMAND=(
            python experimental/jobs/qdm_sec7_fixed_o1_sequence.py
            --output-dir "${OUTPUT_DATA_DIR}"
        )
        SOLVE_POLICY="solver-free assembly of homogeneous Lx=4,8,12 fixed-window sequence"
        ;;
    status)
        JOB_COMMAND=(
    ...
    *)
        ...
        echo "expected one of: status, target-block-status, target-block-refine, fixed-O1-pilot, fixed-O1-L12-spectrum, fixed-O1-L12-observables, fixed-O1-three-size" >&2
        exit 2
        ;;
esac
...
Lx=12 fixed-window budgets: ${FIXED_L12_BUDGETS}
Lx=12 solver tolerance: ${FIXED_L12_TOLERANCE}
Lx=12 maximum automatic budget: ${FIXED_L12_MAX_BUDGET}
Lx=12 residual acceptance tolerance: ${FIXED_L12_RESIDUAL_TOLERANCE}
Lx=12 observable budget tolerance: ${FIXED_L12_OBSERVABLE_TOLERANCE}
...
Recommended remaining P0 sequence using one explicit run id:
